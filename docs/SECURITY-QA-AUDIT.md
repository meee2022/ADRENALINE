...
| ~~SEC-03~~ | 🟢 **مُغلق — كان تقديري خاطئاً** | `.env` في تاريخ Git | `git show ca3cc04:.env` | المشروع **لا يستخدم Supabase إطلاقاً**: صفر إشارة في الكود، وليست اعتمادية في `package.json`. المفتاح المسرَّب يخصّ مشروعاً غير موجود. والمتغيّر الآخر `VITE_CONVEX_URL` يُشحَن للمتصفح بحكم بادئة `VITE_` فهو عام بالتصميم. **لا حاجة لتدوير مفاتيح ولا لإعادة كتابة تاريخ Git.** أُزيلت الأسطر الميتة من `.env`. |
...
| **SEC-05** | 🟠 متوسطة | حسابات بهاش `simpleHash` (دالة 6 أسطر، غير مالحة) | **✅ حساب الـADMIN أُصلح 2026-07-10** (كل `users` صارت PBKDF2). **متبقٍّ 3 في `customerAccounts`:** `talalhawk@outlook.com` · `ABEERABDELHAMEED12345@gmail.com` · `eng.mohamdkamal1987@gmail.com` | كسر فوري عند تسرّب قاعدة البيانات. الخطر انخفض كثيراً بعد تأمين الـADMIN؛ المتبقّون عملاء لا موظفون |
...
| # | الإجراء | الحالة |
|---|---|---|
| 1 | إضافة ترويسات الأمان + CSP بوضع Report-Only (§5.1) | ✅ **منفّذ** — الست ترويسات حيّة |
| 2 | ~~تدوير مفتاح Supabase + `git filter-repo`~~ | 🟢 **مُلغى** — Supabase غير مستخدم أصلاً (انظر SEC-03) |
| 3 | إعادة تعيين كلمات المرور ذات الهاش القديم | 🟡 **جزئي** — الـADMIN تمّ. متبقٍّ 3 حسابات عملاء |
| 4 | حدّ محاولات تسجيل الدخول (§5.2) | ✅ **منفّذ** — يُحظر بعد 5 محاولات / 15 دقيقة |
| 5 | حدّ معدّل على `ai.chat` / `ai.generateSmartPlan` | ✅ **منفّذ** — دلو لكل رقم + سقف عام |
| 6 | حذف `server/` + `shared/schema.ts` الميتة | ✅ **منفّذ** — 144 خطأ نوع → 0 · ثغرات الإنتاج 9 → 1 |

> **الباقي فعلياً:** إعادة تعيين كلمات مرور الحسابات الثلاثة، وتفعيل الـCSP بعد المراقبة.
