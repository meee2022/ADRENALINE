...
export default function BannersManagement() {
  ...
  const [target, setTarget] = useState<"web" | "app" | "both">("app");
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <select value={target} onChange={(e) => setTarget(e.target.value as typeof target)} className="mt-1 w-full h-10 rounded-md border border-slate-300 bg-white px-3 text-sm">
                  <option value="app">{t("الهيرو — التطبيق والموقع", "Hero — app and website")}</option>
                  <option value="web">{t("قديم: صور الموقع (لم تعد تُعرض)", "Legacy: website photos (no longer shown)")}</option>
                </select>
              </div>
              {target !== "web" && (
                <div>
                  <Label>{t("نوع الشريحة في الهيرو", "Hero slide type")}</Label>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        <span className="text-xs px-2.5 py-0.5 rounded-full font-semibold bg-cyan-50 text-cyan-800">
                          {banner.target === "app" || banner.target === "both" ? t("الهيرو", "Hero") : t("قديم — غير معروض", "Legacy — not shown")}
