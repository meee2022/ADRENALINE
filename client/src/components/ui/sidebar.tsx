...
function SidebarProvider({
  ...
  // ✅ استخرج اتجاه الصفحة من الـ DOM (index.html عندك dir="rtl")
  const dir: "rtl" | "ltr" =
    (typeof document !== "undefined" &&
      (document.documentElement.getAttribute("dir") as "rtl" | "ltr")) ||
    "rtl";
...
function Sidebar({
  ...
  // ✅ في RTL الافتراضي الأفضل تكون الـ sidebar على اليمين
  const resolvedSide = (isRtl ? "right" : side) as "left" | "right";
...
function SidebarInput({
  ...
  return (
    <Input
      ...
      data-sidebar="input"
      className={cn(
        "bg-background h-8 w-full shadow-none text-right",
        className,
      )}
      {...props}
    />
...
const sidebarMenuButtonVariants = cva(
  // ✅ أهم تعديل: text-left -> text-right + دعم RTL للأيقونة/النص
  "peer/menu-button flex w-full flex-row-reverse items-center gap-2 overflow-hidden rounded-md p-2 text-right text-sm outline-hidden ring-sidebar-ring transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pl-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:font-medium data-[active=true]:text-sidebar-accent-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:w-8! group-data-[collapsible=icon]:h-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
  {
    variants: {
      variant: {
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
  ...
  return (
    <ul
      ...
      className={cn(
        // ✅ RTL: border-r بدل border-l
        "border-sidebar-border mx-3.5 flex min-w-0 flex-col gap-1 px-2.5 py-0.5",
...
function SidebarMenuSubButton({
  ...
  return (
    <Comp
      ...
      data-active={isActive}
      className={cn(
        "text-sidebar-foreground ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground active:bg-sidebar-accent active:text-sidebar-accent-foreground [&>svg]:text-sidebar-accent-foreground flex h-7 min-w-0 flex-row-reverse items-center gap-2 overflow-hidden rounded-md px-2 text-right outline outline-2 outline-transparent outline-offset-2 focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
