...
export default function MealDetail() {
  ...
  const all = (useQuery(api.publicMeals.listMeals, {}) || []) as any[];
  const meal = all.find((m) => String(m._id) === String(id));

  return (
    <View style={{ flex: 1, backgroundColor: "#fff" }}>
      <ScrollView contentContainerStyle={{ padding: 16, paddingTop: insets.top + 8, paddingBottom: insets.bottom + 90 }} showsVerticalScrollIndicator={false}>
        <View style={styles.imgWrap}>
          {meal?.imageUrl ? <Image source={{ uri: meal.imageUrl }} style={StyleSheet.absoluteFill} contentFit="cover" transition={200} /> : null}
          <Pressable onPress={() => router.back()} style={styles.close} hitSlop={8}><Ionicons name="close" size={20} color={colors.navy2} /></Pressable>
          {meal ? (
        ...
        {meal ? (
          <>
            <T w="bold" style={styles.catLine}>
              {customerCategoryLabel(meal.category, true)}
              {meal.tags?.length ? <T w="semibold" style={{ color: colors.muted, fontSize: 12 }}> · {meal.tags.slice(0, 3).join(" · ")}</T> : null}
            </T>
            <T w="black" style={styles.name}>{meal.nameAr}</T>
            {meal.nameEn ? <T style={{ color: colors.muted, fontSize: 13 }}>{meal.nameEn}</T> : null}

            {meal.descriptionAr ? (
              <View style={{ marginTop: 18 }}>
                <T w="bold" style={styles.label}>الوصف</T>
                <T style={{ fontSize: 14.5, lineHeight: 24, color: "#3D4F5C" }}>{meal.descriptionAr}</T>
              </View>
            ) : null}
              ...
              <View style={{ flexDirection: "row", gap: 8 }}>
                {[[meal.calories, "", "سعرة"], [meal.protein, "g", "بروتين"], [meal.carbs, "g", "كارب"], [meal.fats, "g", "دهون"]].map(([v, u, l], i) => (
                  <View key={i} style={styles.tile}>
                    <T w="black" style={{ fontSize: 19, color: colors.navy2, fontVariant: ["tabular-nums"] }}>{v ?? 0}<T w="bold" style={{ fontSize: 12, color: colors.muted2 }}>{u as string}</T></T>
          ...
          <T style={{ color: colors.muted, textAlign: "center", padding: 30 }}>جارٍ التحميل…</T>
        )}
      </ScrollView>

      <View style={[styles.footer, { paddingBottom: insets.bottom + 12 }]}>
        <T w="semibold" style={{ fontSize: 12, color: colors.muted, flex: 1 }}>ضمن اشتراكك</T>
...
const styles = StyleSheet.create({
  ...
  close: { position: "absolute", top: 10, end: 10, width: 34, height: 34, borderRadius: 17, backgroundColor: "rgba(255,255,255,0.92)", alignItems: "center", justifyContent: "center" },
